...
function LoginPage () {
  ...
  const onSubmit = async (event) => {
    ...
    if (postDataForm.status !== 'success' || postDataForm === undefined) {
      swal({
        ...
        'text': `${local === 'id' ? 'Cek kembali email dan kata sandi anda!' : 'Check your email and password again!'}`,
      }).then(() => {
        navigate('/');
      });
      return;
  ...
  return (
    <>
      <Card className={`${theme === 'light' ? 'bg-dark' : 'bg-light'}`} style={{margin: '0 auto', border: 'none'}}>
        <Card.Header className="bg-primary">
          <h2 className="text-center">{local === 'id' ? 'Form Login' : 'Login Form'}</h2>
        </Card.Header>
          ...
          <form className="container" onSubmit={onSubmit} onReset={onReset}>          
            ...
            <div className="mt-3">
              <Button theme="blue" buttonOptions={{
                type: 'submit',
              }}>{local === 'id' ? 'Masuk' : 'Login'}</Button>
              <Button theme="red" buttonOptions={{
                type: 'reset',
              }}>{local === 'id' ? 'Bersihkan' : 'Reset'}</Button>             
            </div>
            <div className="mt-2">
              <p className="text-center">{local === 'id' ? 'Belum punya akun? ' : 'Doesn\'t Have an account? '}<Link className="text-primary" to="/register" title="halaman registrasi">{local === 'id' ? 'Registrasi disini! ' : 'Register here!'}</Link></p>
            </div>
          </form>
        </Card.Body>
      </Card>
