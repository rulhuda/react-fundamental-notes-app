...
function RegisterPage() {
  ...
  const onSubmit = async (event) => {
    ...
    if (
      formControl.password.length === 0 ||
      formControl.passwordConfirmation.length === 0
    ) {
      return;
    }
    ...
    if (formControl.password !== formControl.passwordConfirmation) {
      swal({
        icon: "warning",
        title: `${
          local === "id" ? "Kata sandi tidak sama!" : "Password doesn't match!"
        }`,
    ...
    if (postDataForm.status !== "success" || postDataForm === undefined) {
      swal({
        icon: "warning",
        title: `${
          local === "id" ? "Registrasi gagal!" : "Registration failed!"
        }`,
        text: `${
          local === "id"
    ...
    swal({
      icon: "success",
      title: `${
        local === "id" ? "Registrasi berhasil!" : "Registration succesfully!"
      }`,
      text: `${
        local === "id"
          ? "Silahkan login dengan akun yang telah anda daftarkan!"
          : "Please login with the account you have registered!!"
      }`,
    }).then(() => {
      navigate("/");
    });
    ...
    <>
      ...
      >
        <Card.Header className='bg-primary'>
          ...
          >
            {local === "id" ? "Form Registrasi" : "Registration Form"}
          </h2>
        </Card.Header>
        <Card.Body
          className={`${theme === "light" ? "bg-light" : "bg-dark"}`}
        ...
        >
          <form className='container' onSubmit={onSubmit} onReset={onReset}>
            ...
            <div className='mt-2'>
              ...
              >
                {local === "id" ? "Sudah punya akun? " : "Have an account? "}{" "}
                <Link className='text-primary' to='/' title='halaman login'>
                  {local === "id" ? "Login disini! " : "Login here! "}
                </Link>
              </p>
            </div>
          </form>
